...
def mlParams(X, labels, W=None):
    ...
    for k, class_name in enumerate(classes):
        ...
        id = labels == class_name  # Returns a true or false with the length of y
        id = np.where(labels == class_name)[0]  # Return element depending on the condition given
        #lc = X[id, :]  # Get the x for the class labels. Vectors are rows.
        #Weight Computation

        weight_sum = sum(W[id, :])

        #Mean
        mu[k] += np.sum(X[id,:]*W[id,:], axis=0) / weight_sum
        # Sigma - contains the covariance
        for dim in range(Ndims):
            SUM = 0
            for ind in id:
                SUM += W[ind] * (X[ind][dim] - mu[k][dim]) ** 2
            sigma[k][dim][dim] = SUM / weight_sum
...
# ===========================================================================================
# TEST OF ASSIGNMENT 1
# X, labels = genBlobs(n_samples=200, centers=6)
# mu, sigma = mlParams(X, labels)
# plotGaussian(X, labels, mu, sigma)

# TEST OF ASSIGNMENT 4
# X, labels = genBlobs(n_samples=200, centers=5)
# mu, sigma = mlParams(X, labels)
# plotGaussian(X, labels, mu, sigma)
...
def computePrior(labels, W=None):
    ...
    for k, class_names in enumerate(labels): # Indice like in the subject : Class k
        prior[class_names] += W[k] # Work for 2.1 & 5.1
...
testClassifier(BayesClassifier(), dataset='vowel', split=0.1)
...
plotBoundary(BayesClassifier(), dataset='vowel', split=0.9)

# 0.1
# Trial: 90 Accuracy 83.7
# Final mean classification accuracy  86.8 with standard deviation 4.67

# 0.3
# Trial: 90 Accuracy 88.6
# Final mean classification accuracy  89.2 with standard deviation 2.97
...
# Final mean classification accuracy  88.9 with standard deviation 8.44
...
def trainBoost(base_classifier, X, labels, T=10):
    ...
    w_new = np.ones((Npts, 1)) / float(Npts)

    for i_iter in range(0, T):
        # a new classifier can be trained like this, given the current weights
        classifiers.append(base_classifier.trainClassifier(X, labels, w_new))

        # do classification for each point
        vote = classifiers[-1].classify(X)

        # Compute error by classes to simplify operations
        weight_sum = np.sum(w_new)
        ...
        # Adaboost step (3)
        Alpha_t = (np.log(1 - error) - np.log(error)) / 2  # Compute new alpha
        alphas.append(Alpha_t)  # you will need to append the new alpha

        # Adaboost step (4) : Normalization of the weights so that their sum is equal to 1
        false_vote = np.where(vote != labels)[0]
        w_Old = w_new
        # Updating the new weight
        for i in correct_vote:
            w_new[i] = w_Old[i] * np.exp(-Alpha_t)
        for i in false_vote:
            w_new[i] = w_Old[i] * np.exp(Alpha_t)
        w_new /= np.sum(w_new)
...
def classifyBoost(X, classifiers, alphas, Nclasses):
    ...
    if Ncomps == 1:
        ...
        for id, classifier in enumerate(classifiers):
            ...
            for i in range(Npts):
                votes[i][classified[i]] += alphas[id]

        #classifiers[t].classify(data, labels, W)
