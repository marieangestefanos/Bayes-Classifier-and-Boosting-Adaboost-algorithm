...
def plotGaussian(X,y,mu,sigma):
    ...
    for label in labels:
        ...
        plt.scatter(Xclass[:,0],Xclass[:,1],linewidths=1,s=25,color=colors[label],marker='o',alpha=0.75)
        c += 1.
    plt.xlabel('Gaussian estimation on X1')
    plt.ylabel('Gaussian estimation on X2')
    plt.title('Bayesian Learning with 3 blobs of 100 samples')
    plt.show()
...
def plotBoundary(classifier, dataset='iris', split=0.7):
    ...
    plt.xlabel('M.L. estimation on X1')
    plt.ylabel('M.L. estimation on X2')
    plt.title('Maximum Likelihood (Boosted Classifier) estimations on iris dataset')
    plt.show()
